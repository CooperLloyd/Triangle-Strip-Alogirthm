...
def buildTristrips(triangles):
    ...
    while True:  # change to while not complete

        # Deciding Strip Start point
        i = 0  # Index used for start of strip
        bestSoFar = 4  # Integer used to hold lowest number of adjacent triangles
        bestSoFarIndex = -1  # Integer to hold index of best triangle

        # Loop through triangles list until end or until start point with one adjacent triangle is found
        while i < (len(triangles) - 1) and bestSoFar > 1:
            i += 1
            # Find lowest number of adjacent triangles and store that number and its index
            if len(triangles[i].adjTris) == 1 and triangles[i].nextTri is None and triangles[i].prevTri is None:
                bestSoFar = 1
                bestSoFarIndex = triangles[i].id
            elif bestSoFar > 2 and len(triangles[i].adjTris) == 2 and triangles[i].nextTri is None and triangles[
                i].prevTri is None:
                bestSoFar = 2
                bestSoFarIndex = triangles[i].id
            elif bestSoFar > 3 and len(triangles[i].adjTris) == 3 and triangles[i].nextTri is None and triangles[
                i].prevTri is None:
                bestSoFar = 3
                bestSoFarIndex = triangles[i].id

        # If there were no better triangles, end the loop
        if bestSoFar > 3:
            break

        # Store the start point into i
        i = bestSoFarIndex
        ...
        while True:
            ...
            num = choose_next(triangles, i)
            new_connection(triangles, i, num)
            remove_adj_connections(triangles, i)

            # If there are is a new nextTri, set i to that triangle, otherwise break and increase strip count
            if triangles[i].nextTri.adjTris:
                i = triangles[i].nextTri.id
            else:
                count += 1
                break
...
# Loop through the triangle's adjacent triangles and remove those inward connections, as well as all of that
# triangles' outward connections
def remove_adj_connections(triangles, id):
    for i in range(len(triangles[id].adjTris)):
        triangles[id].adjTris[i].adjTris.remove(triangles[id])
    triangles[id].adjTris.clear()


# Creates a new double linked list connection
def new_connection(triangles, i, num):
    triangles[i].nextTri = triangles[i].adjTris[num]
    triangles[i].adjTris[num].prevTri = triangles[i]


# Chooses the best next triangle. If there are less than one, select the one. Otherwise compare all other connections
# to choose the triangle with the lowest number of adjacent triangles
def choose_next(triangles, id):
    bestTriangle = 0
    if len(triangles[id].adjTris) > 1:
        for i in range(1, len(triangles[id].adjTris)):
            if len(triangles[id].adjTris[i].adjTris) < len(triangles[id].adjTris[bestTriangle].adjTris):
                bestTriangle = i
